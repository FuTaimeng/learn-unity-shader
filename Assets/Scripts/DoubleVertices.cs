...
public class DoubleVertices : MonoBehaviour
{
	...
	void IncreseVertices()
	{
		...
		for (int i = 0; i < k; i++)
		{
			...
			if (AddVertices(verticesResultDic, trangle[0], index, uvs)) index++;
			if (AddVertices(verticesResultDic, trangle[1], index, uvs)) index++;
			if (AddVertices(verticesResultDic, trangle[2], index, uvs)) index++;

			if (AddVertices(verticesResultDic, v01, index, uvs)) index++;
			if (AddVertices(verticesResultDic, v12, index, uvs)) index++;
			if (AddVertices(verticesResultDic, v02, index, uvs)) index++;
	...
	bool AddVertices(Dictionary<Vector3, int> verticesResultDic, Vector3 vertice, int index, List<Vector2> uvs)
	{
	...
	{
		return new List<Vector3>(verticesResultDic.Keys).ToArray();
	}
}